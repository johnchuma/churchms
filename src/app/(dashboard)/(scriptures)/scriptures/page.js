...
import { deleteScripture, getScripture, getScriptures } from "@/app/controllers/scriptures_controller";
    ...
    return ( <div>
        ...
        <div className="flex justify-between items-start mt-5  ">
            ...
            <Link href={`/addAnnouncement/`} className="bg-indigo-600 text-white
             py-2 px-3 text-base font-bold rounded-lg">Add announcement</Link>
        </div>
        

        {
             scriptures&&<div className=" py-0 text-start bg-white mt-4">
            ...
            {<table className="min-w-full  ">
                 <thead className="font-medium border-b border-slate-200 py-3 ">
                     ...
                     <th className="text-start py-2 text-base text-slate-900 font-medium">Book</th>
                     <th className="text-start py-2 text-base text-slate-900 font-medium">Words</th>             
                     <th className="text-start py-2 text-base text-slate-900 font-medium"></th>
                     <th className="text-start py-2 text-base text-slate-900 font-medium"></th>
                 </thead>
                 <tbody className="space-y-2">
                    {scriptures.filter((item)=>`${item.name}`.toLowerCase().includes(keyword.toLowerCase())).filter((item, index) => index + 1 > ((currentPage - 1) * itemsPerPage) && index + 1 <= (currentPage*itemsPerPage))
                    .map((item,key)=>{
                     return <tr className="" key={key}>
                     <td className="py-2 text-base">{timeAgo(item.createdAt.toDate())}</td>
                     <td className="py-2 text-base">{item.book}</td>
                     <td className="py-2 text-base">{item.words}</td>
                     <td  className=" text-indigo-600 bg-opacity-20  font-bold">
                         <Link href={`/editScripture/${item.id}`}>
                     ...
                     <td  className=" text-danger bg-opacity-20 font-bold">
                         <div className="cursor-pointer" onClick={()=>{
                            deleteScripture(item.id).then((data)=>{
