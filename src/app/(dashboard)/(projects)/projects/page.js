...
import { deleteProject, getProjects } from "@/app/controllers/projects_controller";
...
const Page = ({params}) => {
    ...
    return ( <div>
        ...
        <div className="flex justify-between items-start mt-5  ">
            ...
            <h1 className='text-2xl font-medium text-slate-900'>Projects</h1>
            </div>
            <Link href={`/addProject/`} className="bg-indigo-600 text-white
             py-2 px-3 text-base font-medium rounded-lg">Add project</Link>
        </div>
        

        {
             projects&&<div className=" py-0 text-start bg-white mt-4">
            ...
            {<table className="min-w-full  ">
                 <thead className="font-medium border-b border-slate-200 py-3 ">
                     ...
                     <th className="text-start py-2 text-base text-slate-900 font-medium">Project</th>            
                     <th className="text-start py-2 text-base text-slate-900 font-medium"></th>
                     <th className="text-start py-2 text-base text-slate-900 font-medium"></th>
                 </thead>
                 <tbody className="space-y-2">
                    {projects.filter((item)=>`${item.name}`.toLowerCase().includes(keyword.toLowerCase())).filter((item, index) => index + 1 > ((currentPage - 1) * itemsPerPage) && index + 1 <= (currentPage*itemsPerPage))
                    .map((item,key)=>{
                     return <tr className="" key={key}>
                     <td className="py-2 text-base">{timeAgo(item.createdAt.toDate())}</td>
                     <td className="py-2 text-base">{item.project}</td>
                     <td  className=" text-indigo-600 bg-opacity-20  font-medium">
                         <Link href={`/editProject/${item.id}`}>
