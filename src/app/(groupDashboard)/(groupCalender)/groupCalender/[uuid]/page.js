...
import { getGroupCalenders } from "@/app/controllers/group_calender_controller";

const Page = ({params}) => {
    const [groupcalenders,setGroupCalenders] = useState([])
    ...
    useEffect(()=>{
       setLoading(true)
       getGroupCalenders(params.uuid).then((data)=>{
            setGroupCalenders(data)
    ...
    return ( <div>
        ...
        <div className="flex justify-between items-start mt-5  ">
            ...
            <h1 className='text-2xl font-medium text-slate-900'>Calender of {group&&group.name}</h1>
            </div>
            <Link href={`/addGroupCalender/${params.uuid}`} className="bg-indigo-600 text-white
             py-2 px-3 text-base font-medium rounded-lg">Add calender event</Link>
        </div>
                 ...
                 <tbody className="space-y-2">
                    ...
                    .map((item,key)=>{
                     ...
                     <td  className=" text-indigo-600 bg-opacity-20  font-medium">
                         <Link href={`/editGroupCalender/${item.id}`}>
                     ...
                     <td  className=" text-danger bg-opacity-20 font-medium">
                         <div className="cursor-pointer" onClick={()=>{
                            deleteGroupCalender(item.id).then((data)=>{
