...
import { deleteGroupExpense, getGroupExpenses } from "@/app/controllers/group_expenses_controller";
...
const Page = ({params}) => {
    ...
    useEffect(()=>{
       setLoading(true)
       getGroupExpenses(params.uuid).then((data)=>{
    ...
    return ( <div>
        ...
        <div className="flex justify-between items-start mt-5  ">
            ...
            <Link href={`/addGroupExpenses/${params.uuid}`} className="bg-indigo-600 text-white
             py-2 px-3 text-sm font-bold rounded-lg">Add expense</Link>
        </div>
        

        {
             groupexpenses&&<div className=" py-0 text-start bg-white mt-4">
            ...
            {<table className="min-w-full  ">
                 ...
                 <tbody className="space-y-2">
                    {groupexpenses.filter((item)=>`${item.name}`.toLowerCase().includes(keyword.toLowerCase())).filter((item, index) => index + 1 > ((currentPage - 1) * itemsPerPage) && index + 1 <= (currentPage*itemsPerPage))
                    .map((item,key)=>{
