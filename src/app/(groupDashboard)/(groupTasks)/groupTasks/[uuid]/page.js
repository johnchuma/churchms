...
import { getGroupTasks } from "@/app/controllers/group_tasks_controller";
...
const Page = ({params}) => {
    ...
    useEffect(()=>{
       setLoading(true)
       getGroupTasks(params.uuid).then((data)=>{
            setGroupTasks(data)
    ...
    return ( <div>
        ...
        <div className="flex justify-between items-start mt-5  ">
            ...
            <h1 className='text-2xl font-bold text-slate-900'>Tasks of {group&&group.name}</h1>
            </div>
            <Link href={`/addGroupTask/${params.uuid}`} className="bg-indigo-600 text-white
             py-2 px-3 text-sm font-bold rounded-lg">Add task</Link>
        </div>
        

        {
             grouptasks&&<div className=" py-0 text-start bg-white mt-4">
            ...
            {<table className="min-w-full  ">
                 ...
                 <tbody className="space-y-2">
                    {grouptasks.filter((item)=>`${item.name}`.toLowerCase().includes(keyword.toLowerCase())).filter((item, index) => index + 1 > ((currentPage - 1) * itemsPerPage) && index + 1 <= (currentPage*itemsPerPage))
                    .map((item,key)=>{
                     ...
                     <td  className=" text-indigo-600 bg-opacity-20  font-bold">
                         <Link href={`/editGroupTask/${item.id}`}>
                     ...
                     <td  className=" text-danger bg-opacity-20 font-bold">
                         <div className="cursor-pointer" onClick={()=>{
                            deleteGroupTask(item.id).then((data)=>{
